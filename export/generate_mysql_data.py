...
def insert_city(nums):
    existed = set()
    while len(existed) < nums:
        city = fake.city()
        existed.add(city)
    for _city in existed:
        model = City()
        model.city_name = _city
        model.province_id = random.randint(1, 29)
        model.save()
...
def insert_user(thread_name, nums):
    ...
    for _ in range(nums):
        ...
        create_time = fake.date_time_between_dates(
            datetime_start=datetime(1993, 1, 1, 0, 0, 0),
            datetime_end=datetime(2010, 8, 12, 0, 0, 0, 0))
...
for i in range(thread_nums):
    t = threading.Thread(target=insert_user, args=(i, 1000))
    t.start()
